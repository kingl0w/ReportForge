...
export default function SignupPage() {
  ...
  async function handleEmailSignup(e: React.FormEvent) {
    ...
    try {
      const checkRes = await fetch("/api/auth/signup-check", { method: "POST" });
      const checkData = await checkRes.json() as { allowed: boolean; message?: string };
      if (!checkData.allowed) {
        setError(checkData.message ?? "Too many signups. Please try again later.");
        setLoading(false);
        return;
      }
    } catch {
      //if the rate-limit check fails, allow signup to proceed
    }
